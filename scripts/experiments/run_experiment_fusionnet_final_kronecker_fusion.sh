...
### THIS IS AN EXPERIMENTAL APPROACH. DOES NOT WORK
# get dir
scripts_dir=`pwd`
...
if [[ $(basename "$VIRTUAL_ENV") == "$virtual_env_name" ]]; then
  ...
  do
    ...
    --experiment-tag="Fusionnet_kronecker" --comet-api-key="$COMET_API_KEY" --comet-project-name="final_models" --comet-workspace="$COMET_WORKSPACE" \
