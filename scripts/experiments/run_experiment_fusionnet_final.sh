...
### In this experiment, we fine-tune the fusion network with the best hyperparams
# get dir
scripts_dir=`pwd`
...
if [[ $(basename "$VIRTUAL_ENV") == "$virtual_env_name" ]]; then
  ...
  do
    echo "ROUND $i \n\n"
    # the full network
    python3 train.py --steps=5000 --val-steps=500 --epochs=50 --batch-size=1 \
    --experiment-tag="FUSIONNET" --comet-api-key="$COMET_API_KEY" --comet-project-name="final_models" --comet-workspace="$COMET_WORKSPACE" \
    ...
    --config $scripts_dir/test_config/transformer_config.ini \
    --no-evaluation
    #--pretrained-snapshot "/home/abawi/multitask_grasping/modules/language_translation/transformer/snapshots/58b5f2c937764a899e6529115dc124c9/transformer.h5" \
