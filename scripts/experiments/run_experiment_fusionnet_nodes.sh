...
### In this experiment, we try different node dimensions
# get dir
scripts_dir=`pwd`
...
if [[ $(basename "$VIRTUAL_ENV") == "$virtual_env_name" ]]; then
  ...
  do
    echo "ROUND $i \n\n"
    # vary the node dimension {1, 2, 4, 8, 16, 20, 25, 32,64}
    for n in 1 2 4 8 16 20 25 32 64
    do
      python3 train.py --steps=5000 --val-steps=500 --no-snapshots --epochs=10 --batch-size=1 \
      --experiment-tag "NODES_$n" --comet-api-key="$COMET_API_KEY" --comet-project-name="ir_fusionnet_nodes_exp" --comet-workspace="$COMET_WORKSPACE" \
